...
if __name__ == "__main__":
    ...
    while not optimal_num_blocks_found:
        ...
        block_assignment, num_blocks = carry_out_best_merges(delta_entropy_for_each_block, best_merge_for_each_block, block_assignment,
        ...
        for itr in range(max_num_nodal_itr):
            ...
            for current_node in range(N):
                current_block = block_assignment[current_node]
                # propose a new block for this node
                proposal, num_out_neighbor_edges, num_in_neighbor_edges, num_neighbor_edges = propose_new_partition(
                    current_block, out_neighbors[current_node], in_neighbors[current_node], block_assignment,
                ...
                if (proposal != current_block):
                    ...
                    blocks_out, inverse_idx_out = np.unique(block_assignment[out_neighbors[current_node][:, 0]],
                                                            return_inverse=True)
                    count_out = np.bincount(inverse_idx_out, weights=out_neighbors[current_node][:, 1]).astype(int)
                    blocks_in, inverse_idx_in = np.unique(block_assignment[in_neighbors[current_node][:, 0]], return_inverse=True)
                    ...
                    if (np.random.uniform() <= p_accept):
                        ...
                        block_assignment, interblock_edge_count, block_degrees_out, block_degrees_in, block_degrees = update_partition(
                            block_assignment, current_node, current_block, proposal, interblock_edge_count,
        ...
        if visualize_graph:
            graph_object = plot_graph_with_partition(out_neighbors, block_assignment, graph_object)
        ...
        block_assignment, interblock_edge_count, block_degrees, block_degrees_out, block_degrees_in, num_blocks, num_blocks_to_merge, old_block_assignment, old_interblock_edge_count, old_block_degrees, old_block_degrees_out, old_block_degrees_in, old_overall_entropy, old_num_blocks, optimal_num_blocks_found = \
            prepare_for_partition_on_next_num_blocks(overall_entropy, block_assignment, interblock_edge_count, block_degrees,
                                                    block_degrees_out, block_degrees_in, num_blocks, old_block_assignment,
    ...
    evaluate_partition(true_partition, block_assignment)
